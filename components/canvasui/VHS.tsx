...
export interface VHSElements {
  ...
  /** Image played as the tape instead of the captured HTML subtree. */
  imageSrc?: string;
  /**
   * Extra drawing composited over the image every frame (image mode only), so
   * the shader distorts it together with the tape.
   */
  composite?: VHSComposite;
}
...
export function createVHS(
  ...
  const { source, content, output, imageSrc } = elements;
  const composite = elements.composite ?? null;
  ...
  // An image source plays straight off a 2D canvas, so the tape runs everywhere
  // instead of only in browsers that expose html-in-canvas.
  const imageMode = Boolean(imageSrc) && Boolean(sourceCtx);
  const htmlInCanvas =
    !imageMode &&
    Boolean(
      sourceCtx &&
  ...
  if (htmlInCanvas) {
    paintable.onpaint = () => {
      try {
        ...
        // A failed capture leaves the previous frame on screen.
      }
    };
  }
  ...
  const image = imageMode ? new Image() : null;
  let imageReady = false;

  function paintSource() {
    if (!sourceCtx) return;
    if (!imageReady && !composite) return;
    // Composite frames redraw every tick, so they stay at CSS resolution to
    // keep the per-frame texture upload cheap; the shader output still runs
    // at full device resolution.
    const scale = composite ? 1 : Math.min(window.devicePixelRatio || 1, 2);
    ...
    if (composite) composite(sourceCtx, width, height, time);
    contentDirty = true;
  }
  ...
  function uploadContent() {
    if (!contentDirty || (!htmlInCanvas && !imageMode)) return;
    contentDirty = false;
    syncBezelColor();
    gl!.bindTexture(gl!.TEXTURE_2D, contentTexture);
    gl!.texImage2D(
  ...
  function render() {
    if (imageMode && composite) paintSource();
    uploadContent();
  ...
  if (!imageMode) observer.observe(content);
...
export interface VHSProps extends VHSOptions {
  children?: ReactNode;
  /** Image played as the tape instead of the DOM children. */
  imageSrc?: string;
  /** Per-frame drawing composited over the image and distorted with it. */
  composite?: VHSComposite;
  className?: string;
...
export function VHS({
  ...
  const compositeRef = useRef<VHSComposite | undefined>(composite);
  const [initialOptions] = useState(options);
  const [failed, setFailed] = useState(false);
  const hasComposite = Boolean(composite);

  // A stable trampoline lets the callback change without tearing down the
  // whole GL instance.
  const [compositeProxy] = useState(
    (): VHSComposite =>
      (ctx, width, height, time) => {
        compositeRef.current?.(ctx, width, height, time);
      },
  );
  ...
  const native = !imageSrc && supported && !failed;
    ...
    instanceRef.current = createVHS(
      {
        ...
        output,
        imageSrc,
        composite: hasComposite ? compositeProxy : undefined,
      },
      initialOptions,
    );
  ...
  }, [initialOptions, native, imageSrc, hasComposite, compositeProxy]);
