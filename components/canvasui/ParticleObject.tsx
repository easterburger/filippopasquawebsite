...
export interface ParticleObjectOptions {
  ...
  /**
   * Called with how hard the cursor is stirring the cloud, from 0 to 1, as it
   * changes. Combines how much of the cloud is inside the push field with how
   * fast the cursor is moving through it, so it can drive sound or effects that
   * should only respond to actual disturbance.
   */
  onActivity?: ((activity: number) => void) | null;
}
...
export function createParticleObject(
  ...
  /** Returns the share of the cloud sitting inside the push field, 0 to 1. */
  function simulate(delta: number) {
    if (!points || !homes || !velocities || particleCount === 0) return 0;
    ...
    return reached / particleCount;
  }

  let activity = 0;
  let reportedActivity = 0;

  function reportActivity(reached: number, delta: number) {
    if (!config.onActivity) return;

    // Even a cursor buried in the cloud only covers a slice of it, so the raw
    // share is scaled up before being treated as a 0-to-1 amount.
    const coverage = Math.min(reached * 16, 1);
    const motion = Math.min(pointerSpeed / 1100, 1);
    // Standing still over the cloud barely counts: what matters is movement
    // through it, the same way sand is quiet until something drags across it.
    const target = pointerActive ? coverage * (0.15 + 0.85 * motion) : 0;

    activity += (target - activity) * Math.min(delta * 14, 1);
    if (activity < 0.002) activity = 0;

    const settled = activity === 0 && reportedActivity !== 0;
    if (!settled && Math.abs(activity - reportedActivity) < 0.004) return;
    reportedActivity = activity;
    config.onActivity(activity);
  }

  function silenceActivity() {
    if (reportedActivity === 0) return;
    activity = 0;
    reportedActivity = 0;
    config.onActivity?.(0);
  }
  ...
  function tick(time: number) {
    ...
    if (delta > 0) reportActivity(simulate(delta), delta);
  ...
  function stopLoop() {
    ...
    // Nothing will report the wind-down once the loop is parked.
    silenceActivity();
  }
